    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="dashboard-card xl:col-span-2">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
          <div>
            <p className="text-sm font-semibold text-white">Registered Patients</p>
          ...
          {filteredPatients.length === 0 ? (
            ...
            filteredPatients.map((patient) => (
              <button
                ...
                className="w-full text-left p-4 rounded-2xl bg-slate-900/50 border border-slate-700/30 hover:border-accent/40 transition-colors"
              >
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="font-semibold text-white truncate">{patient.name || 'Patient'}</p>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="dashboard-card">
        <div className="flex items-center gap-2 mb-5">
          ...
          <div>
            <p className="text-base font-semibold text-white">Patient Profile</p>
        ...
        {selectedPatient ? (
          <div className="space-y-4">
            <div>
              <p className="text-sm font-semibold text-white">{selectedPatient.name || 'Patient'}</p>
              <p className="text-xs text-slate-500">{selectedPatient.email || selectedPatient.mail || 'No email'}</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              <div className="rounded-2xl border border-slate-700/40 bg-slate-900/60 p-4">
                ...
                <p className="text-xs text-slate-500">Search appointments use the main scheduler or doctor directories to review patient visit history.</p>
              </div>
              <div className="rounded-2xl border border-slate-700/40 bg-slate-900/60 p-4">
                <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-xs text-slate-500">Patient medical documents are only accessible to the patient and assigned doctors.</p>
              </div>
            </div>
          </div>
        ) : (
